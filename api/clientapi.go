...
func ReportAndGetNotify(server string) (map[int64]map[int64][]byte, error) {
	...
	util.LogInfo().Println("ReportAndGetNotify hyperttp decodeData:", decodeData)

	if decodeData.Result() != "success" {
		return nil, err
	}
	messageList := make(map[int64]map[int64][]byte)

	switch decodeData["data"].(type) {
	case map[string]interface{}:
		data := decodeData["data"].(map[string]interface{})
		for accountId, accountData := range data {
			temp := make(map[int64][]byte)
			switch accountData.(type) {
			case map[string]interface{}:
				for userID, messages := range accountData.(map[string]interface{}) {
					...
					temp[uid] = ApiUnparse(userNotify, util.Token)
				}
			}

			_accountId, _ := util.String2Int64(accountId)
			messageList[_accountId] = temp
		}
	...
	util.LogInfo().Println("ReportAndGetNotify  messageList type:", reflect.TypeOf(messageList))
